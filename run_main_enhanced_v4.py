"""
run_main_enhanced_v4.py — Training entry for Time-LLM Enhanced v4

Core changes from v3:
  - DM v4: polyphase multi-scale (unified k=4) replaces avg-pool downsampling
  - C2F v4: expert voting (MAD) + DWT trend constraint replaces simple weighted sum
  - PVDR v4: logistic extreme + Bayesian changepoint replaces 3-sigma
  - AKG v4: threshold-driven gating replaces fixed sigmoid
  - New loss: L_expert (expert correction rate)
  - Loss formula: L_main + warmup * (lambda_consist * L_consist + lambda_gate * L_gate + lambda_expert * L_expert)
"""
...
from models import TimeLLM_Enhanced_v4 as TimeLLM
...
parser = argparse.ArgumentParser(description='Time-LLM Enhanced v4')


# ========================================================================
# Utility functions (same as run_main.py)
# ========================================================================
...
# ========== v3 TAPR parameters (DM + C2F) ==========
parser.add_argument('--use_tapr', action='store_true', default=False,
                    help='Enable DM multi-scale + C2F fusion')
parser.add_argument('--n_scales', type=int, default=5,
                    help='Total number of scales (including baseline)')
parser.add_argument('--downsample_rates', type=str, default='1,2,4,4,4',
                    help='Downsample rates per scale (comma-separated, should match DM polyphase k values)')
parser.add_argument('--lambda_consist', type=float, default=0.1,
                    help='Consistency loss weight')
parser.add_argument('--consistency_mode', type=str, default='hybrid',
                    choices=['soft', 'hard', 'hybrid'],
                    help='Consistency mode: soft (train), hard (infer), or hybrid')
parser.add_argument('--decay_factor', type=float, default=0.5,
                    help='Weight decay for inconsistent scales at inference')

# ========== v3 GRAM parameters (PVDR + AKG) ==========
parser.add_argument('--use_gram', action='store_true', default=False,
                    help='Enable PVDR retrieval + AKG gating')
parser.add_argument('--lambda_gate', type=float, default=0.01,
                    help='Gate regularization loss weight')
parser.add_argument('--similarity_threshold', type=float, default=0.8,
                    help='PVDR base retrieval threshold')
parser.add_argument('--extreme_sigma', type=float, default=3.0,
                    help='Extreme data detection sigma threshold')
parser.add_argument('--extreme_threshold_reduction', type=float, default=0.2,
                    help='Threshold reduction for extreme data')
parser.add_argument('--top_k', type=int, default=5,
                    help='PVDR top-K retrieval')
parser.add_argument('--d_repr', type=int, default=64,
                    help='Pattern representation dimension')
parser.add_argument('--build_memory', action='store_true', default=False,
                    help='Build PVDR memory bank from training data')

# ========== v4 NEW parameters ==========
parser.add_argument('--lambda_expert', type=float, default=0.05,
                    help='Expert correction loss weight (v4)')
parser.add_argument('--mad_threshold', type=float, default=3.0,
                    help='MAD outlier detection multiplier for intra-scale voting (v4)')
parser.add_argument('--reliability_threshold', type=float, default=0.3,
                    help='Correction rate threshold for unreliable scale flag (v4)')
parser.add_argument('--reliability_penalty', type=float, default=0.3,
                    help='Weight multiplier for unreliable scales (v4)')
parser.add_argument('--cp_min_segment', type=int, default=16,
                    help='Bayesian changepoint detector minimum segment length (v4)')
parser.add_argument('--cp_max_depth', type=int, default=4,
                    help='Bayesian changepoint detector maximum recursion depth (v4)')
...
parser.add_argument('--test_epochs', type=str, default='final',
                    help='"final", "all", or comma-separated like "5,10,15"')
...
parser.add_argument('--warmup_steps', type=int, default=500,
                    help='Warmup steps for auxiliary losses')
...
if accelerator.is_local_main_process:
    print("=" * 70)
    print("Time-LLM Enhanced v4 — Polyphase Multi-scale + Enhanced Retrieval")
    print("=" * 70)
    print(f"  TAPR v4 (DM Polyphase + C2F Voting): {'ENABLED' if args.use_tapr else 'DISABLED'}")
    if args.use_tapr:
        print(f"    n_scales: {args.n_scales}")
        print(f"    downsample_rates: {args.downsample_rates}")
        print(f"    lambda_consist: {args.lambda_consist}")
        print(f"    decay_factor: {args.decay_factor}")
        print(f"    mad_threshold: {args.mad_threshold}")
        print(f"    reliability_threshold: {args.reliability_threshold}")
        print(f"    reliability_penalty: {args.reliability_penalty}")
        print(f"    lambda_expert: {args.lambda_expert}")
    print(f"  GRAM v4 (PVDR Enhanced + AKG Threshold): {'ENABLED' if args.use_gram else 'DISABLED'}")
    if args.use_gram:
        print(f"    top_k: {args.top_k}")
        print(f"    similarity_threshold: {args.similarity_threshold}")
        print(f"    extreme_threshold_reduction: {args.extreme_threshold_reduction}")
        print(f"    lambda_gate: {args.lambda_gate}")
        print(f"    d_repr: {args.d_repr}")
        print(f"    build_memory: {args.build_memory}")
        print(f"    cp_min_segment: {args.cp_min_segment}")
        print(f"    cp_max_depth: {args.cp_max_depth}")
    print(f"  Warmup steps: {args.warmup_steps}")
    print(f"  Test at epochs: {sorted(test_epoch_set)}")
    print("=" * 70)
...
for ii in range(args.itr):
    ...
    if args.swap_val_test:
        ...
        accelerator.print("Note: Validation and test sets swapped")
    else:
    ...
    # ── Build PVDR memory bank ────────────────────────────────────────
    if args.use_gram and args.build_memory:
        accelerator.print("[v4] Building PVDR memory banks (with labels)...")
        unwrapped = accelerator.unwrap_model(model)
        if hasattr(unwrapped, 'build_retrieval_memory'):
            unwrapped.build_retrieval_memory(train_loader, accelerator.device)
        accelerator.print("[v4] Memory banks built")
    ...
    if args.resume_from_checkpoint:
        ...
        if os.path.exists(ckpt_file):
            accelerator.print(f"[v4] Loading checkpoint: {ckpt_file}")
            ckpt = torch.load(ckpt_file, map_location='cpu', weights_only=False)
            model_to_load = accelerator.unwrap_model(model)

            if isinstance(ckpt, dict) and 'model' in ckpt:
                state_dict = ckpt['model']
                # Handle memory bank buffers specially
                memory_buffers = {}
                keys_to_remove = []
                for key in list(state_dict.keys()):
                    if 'memory_bank' in key and ('keys_' in key or 'values_' in key or 'labels_' in key):
            ...
            accelerator.print(f"[v4] Resumed at epoch {start_epoch}, step {global_step}")
            ...
            accelerator.print(f"[v4] Checkpoint not found: {ckpt_file}")
    ...
    for epoch in range(start_epoch, args.train_epochs):
        ...
        resume_skip = resume_step_in_epoch if (
            epoch == start_epoch and resume_step_in_epoch > 0) else 0
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(train_loader)):
            ...
            if args.use_amp:
                with torch.cuda.amp.autocast():
                    ...
                    batch_y_target = batch_y[:, -args.pred_len:, f_dim:].to(
                        accelerator.device)
                    ...
                    if args.use_tapr or args.use_gram:
                        unwrapped = accelerator.unwrap_model(model)
                        aux_loss, aux_dict = unwrapped.compute_auxiliary_loss(
                            batch_y_target, current_step=global_step,
                            warmup_steps=args.warmup_steps)
                        loss = loss + aux_loss
                        aux_loss_list.append(
                            aux_loss.item() if isinstance(aux_loss, torch.Tensor)
                            else aux_loss)
            ...
            # Logging
            if (i + 1) % 100 == 0:
                avg_aux = np.mean(aux_loss_list[-100:]) if aux_loss_list else 0
                accelerator.print(
                    f"\titers: {i + 1}, epoch: {epoch + 1} | "
                    f"loss: {loss.item():.7f} | aux: {avg_aux:.7f}")
            ...
            if args.save_steps > 0 and global_step % args.save_steps == 0:
                if accelerator.is_local_main_process:
                    ...
                    accelerator.print(f"[v4] Saved step checkpoint: {step_dir}")

                    # Limit step checkpoints
        ...
        avg_aux = np.average(aux_loss_list) if aux_loss_list else 0
        ...
        should_test = (current_epoch in test_epoch_set)
        ...
        if should_test:
            test_loss, test_mae_loss = vali(
                ...
                criterion, mae_metric)
            accelerator.print(
                f"Epoch: {current_epoch} | Train: {train_loss:.7f} | "
                f"Aux: {avg_aux:.7f} | Vali: {vali_loss:.7f} | "
                f"Test: {test_loss:.7f} | MAE: {test_mae_loss:.7f}")
        else:
            accelerator.print(
                f"Epoch: {current_epoch} | Train: {train_loss:.7f} | "
                f"Aux: {avg_aux:.7f} | Vali: {vali_loss:.7f}")
        ...
        early_stopping(
            vali_loss, model, path, model_optim, scheduler,
            epoch + 1, global_step,
            rng_state=rng_state, scaler_state=scaler_state,
        ...
        if early_stopping.early_stop:
            ...
            if current_epoch not in test_epoch_set:
                test_loss, test_mae_loss = vali(
                    ...
                    criterion, mae_metric)
                accelerator.print(
                    f"Final Test Loss: {test_loss:.7f} | MAE: {test_mae_loss:.7f}")
            break
        ...
        if args.lradj != 'TST':
            if args.lradj == 'COS':
                scheduler.step()
                accelerator.print(
                    f"lr = {model_optim.param_groups[0]['lr']:.10f}")
            else:
                if epoch == 0:
...
if accelerator.is_local_main_process:
    accelerator.print(f'[v4] Checkpoints saved at: ./checkpoints')
