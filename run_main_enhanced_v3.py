"""
run_main_enhanced_v3.py — Training entry for Time-LLM Enhanced v3

Core changes from v2:
  - DM/C2F/PVDR/AKG modules produce independent predictions fused via matrices
  - Loss: L_main + lambda_consist * L_consistency + lambda_gate * L_gate
  - New hyperparameters for multi-scale prediction and retrieval-augmented fusion
"""
...
parser = argparse.ArgumentParser(description='Time-LLM Enhanced v3')
...
parser.add_argument('--downsample_rates', type=str, default='1,2,4,8,16',
                    help='Downsample rates per scale (comma-separated)')
...
if accelerator.is_local_main_process:
    print("=" * 70)
    print("Time-LLM Enhanced v3 — Multi-scale Prediction + Retrieval Fusion")
    print("=" * 70)
    print(f"  TAPR (DM + C2F): {'ENABLED' if args.use_tapr else 'DISABLED'}")
    if args.use_tapr:
    ...
    print(f"  GRAM (PVDR + AKG): {'ENABLED' if args.use_gram else 'DISABLED'}")
    if args.use_gram:
        print(f"    top_k: {args.top_k}")
        print(f"    similarity_threshold: {args.similarity_threshold}")
        print(f"    extreme_sigma: {args.extreme_sigma}")
...
for ii in range(args.itr):
    ...
    if args.resume_from_checkpoint:
        ...
        if os.path.exists(ckpt_file):
            accelerator.print(f"[v3] Loading checkpoint: {ckpt_file}")
            ...
            accelerator.print(f"[v3] Resumed at epoch {start_epoch}, step {global_step}")
            ...
            accelerator.print(f"[v3] Checkpoint not found: {ckpt_file}")
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(train_loader)):
            ...
            if args.save_steps > 0 and global_step % args.save_steps == 0:
                if accelerator.is_local_main_process:
                    ...
                    accelerator.print(f"[v3] Saved step checkpoint: {step_dir}")
...
if accelerator.is_local_main_process:
    accelerator.print(f'[v3] Checkpoints saved at: ./checkpoints')
